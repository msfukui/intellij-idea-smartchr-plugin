# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## プロジェクト概要

このプロジェクトは、Vimのvim-smartchr機能をIntelliJ IDEAで実現するプラグインです。vim-smartchrは、同じキーを繰り返し押すことで、事前に定義された文字列のシーケンスを循環できる機能です（例：`=`キーを押すたびに `=`、` = `、` == ` と切り替わる）。

## 現在のプロジェクト状態 ✅ 完成

**プロジェクト完成済み（v1.0.0）**
- 全6フェーズ完了、38テスト100%成功
- JSON設定システム、IntelliJエディタ統合、配布準備完了
- 実用的なプラグインとして利用可能

## 開発で学んだ教訓と改善策

### 🏗️ 開発プロセスの改善点

#### 1. API調査の重要性（フェーズ0の提案）
- **問題**: IntelliJ Platform APIの複雑さにより実装で何度も躓いた
- **解決策**: 本格的な実装前に「フェーズ0: API調査・検証」を設ける
- **具体的内容**:
  - 主要APIの動作検証
  - 簡単なプロトタイプ作成
  - 制約事項の洗い出し

#### 2. 段階的テスト戦略
- **成功要因**: JUnit 4への統一とシンプルなテスト構造
- **今後の方針**: 複雑なMockライブラリは避け、シンプルなテストを優先
- **効果**: 38テスト100%成功を維持

### 🔧 技術的な教訓

#### 1. IntelliJ Platform API の罠と対策
- **ファイルエディタ開放問題**: プロジェクト非選択時のファイル開放は`PlatformProjectOpenProcessor`使用
- **UI初期化エラー**: `lateinit property`エラーは`ApplicationManager.invokeLater`で回避
- **設定システム**: XMLからJSONへの移行で使いやすさ大幅向上

#### 2. ユーザビリティ重視の設計
- **UI簡素化**: 複雑なUI → JSONファイル直接編集リンクで大幅改善
- **設定ファイル**: ユーザーが直接編集可能な形式を選択
- **エラーハンドリング**: graceful fallback で操作性確保

### 📝 プロジェクト管理のベストプラクティス

#### 1. ユーザーフィードバックの活用
- **リアルタイム修正**: 問題発見時の即座の方針転換
- **要求仕様の明確化**: 「IntelliJエディタで開く」等の具体的要求重視
- **段階的検証**: 各フェーズでのユーザー確認

#### 2. コミット戦略
- **conventional commit**: 一貫したコミットメッセージで履歴管理改善
- **フェーズ単位コミット**: やり直し可能な粒度での進捗管理
- **品質ゲート**: lint・typecheck必須実行

### 🚀 今後のプロジェクト適用事項

#### 1. 開発フローの標準化
```
フェーズ0: API調査・制約確認 → 各フェーズ: TDD実装 → ユーザー検証 → 修正 → コミット
```

#### 2. 技術選択指針
- IntelliJ Platform: 公式ドキュメント + 実際のコード検証必須
- テスト: シンプル構造優先、複雑なMockは避ける
- UI: ユーザビリティ > 機能豊富さ

#### 3. 品質保証
- 全変更でのlint・typecheck実行
- ユーザー視点での動作確認
- ドキュメント同期更新

## 開発コマンド

Gradleプロジェクトとして初期化後、以下のコマンドを使用：

```bash
# プラグインのビルド
./gradlew build

# プラグインを含むIntelliJ IDEAの起動（テスト用）
./gradlew runIde

# テストの実行
./gradlew test

# ビルド成果物のクリーン
./gradlew clean

# プラグイン配布物の作成
./gradlew buildPlugin
```

## プロジェクト構造（作成予定）

標準的なIntelliJプラグイン構造に従う：

```
├── src/
│   └── main/
│       ├── kotlin/         # Kotlinを使用する場合
│       │   └── com/example/smartchr/
│       │       ├── SmartChrAction.kt
│       │       ├── SmartChrSettings.kt
│       │       └── SmartChrConfigurable.kt
│       └── resources/
│           └── META-INF/
│               └── plugin.xml
├── build.gradle.kts
├── gradle.properties
├── settings.gradle.kts
└── README.md
```

## 実装の重要ポイント

1. **アクションシステム**: IntelliJのアクションシステムを使用してキーストロークを傍受し、循環動作を実装
2. **エディタAPI**: `com.intellij.openapi.editor` APIを使用してテキスト操作を行う
3. **設定画面**: `Configurable`インターフェースを実装し、ユーザーがキーマッピングを設定できるようにする
4. **パフォーマンス**: エディタの遅延を避けるため、キーストローク傍受を効率的に実装

## IntelliJ Platform Plugin要件

- **プラグイン設定**: `plugin.xml`でプラグインのメタデータ、アクション、拡張ポイントを定義
- **互換性**: `build.gradle.kts`で対応するIntelliJバージョンを指定
- **依存関係**: IntelliJ Platform SDKと必要なプラットフォームプラグインを使用

## テスト方針

- 循環ロジックの単体テスト
- IntelliJのテストフレームワークを使用した統合テスト
- サンドボックスIDEでの手動テスト（`./gradlew runIde`経由）

## 重要なAPI

- `AnAction`: アクション実装の基底クラス
- `EditorActionHandler`: エディタ固有のアクション処理
- `TypedActionHandler`: 入力文字の傍受
- `Document`: テキスト操作
- `ApplicationConfigurable`: 設定UI

## コミットメッセージ規約

このプロジェクトではConventional Commits形式を使用します：

```
<type>(<scope>): <subject>

<body>

<footer>
```

### タイプ
- `feat`: 新機能
- `fix`: バグ修正
- `docs`: ドキュメントのみの変更
- `style`: コードの意味に影響を与えない変更（空白、フォーマット、セミコロンなど）
- `refactor`: バグ修正や機能追加を伴わないコード変更
- `test`: テストの追加や修正
- `chore`: ビルドプロセスやツールの変更、ライブラリの更新など

### 例
```
feat(action): vim-smartchrの基本的なキー循環機能を実装
fix(editor): エディタでの文字置換時のカーソル位置を修正
docs: README.mdにインストール手順を追加
```

## コーディング規約

### ファイル形式
- **重要**: すべてのテキストファイルは末尾が改行のみの行で終了すること
- Gitの差分表示で "No newline at end of file" が表示されないようにする
- エディタの設定で自動的に末尾改行を追加するよう設定することを推奨
